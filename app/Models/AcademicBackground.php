...
class AcademicBackground extends Model{
	public function findById(string $id): array {
		$sql = "SELECT * FROM academic_backgrounds WHERE user_id = :user_id AND id = :id AND delete_flg = 0";
	...
	public function list(int $limit = 20): array {
		...
		$sql = "SELECT * FROM academic_backgrounds WHERE user_id = :user_id AND delete_flg = 0";
	...
	public function create(array $posts): PDOStatement | false {
		$sql = "INSERT INTO academic_backgrounds (user_id, name, first_date, last_date, last_career_id, create_at) VALUES (:user_id, :name, :first_date, :last_date, :last_career_id, :create_at)";
		$data = [
	...
	public function update(string $id, array $posts): PDOStatement | false {
		$sql = "UPDATE academic_backgrounds SET name = :name, first_date = :first_date, last_date = :last_date, last_career_id = :last_career_id WHERE user_id = :user_id AND id = :a_id AND delete_flg = 0";
		$data = [
			':a_id'           => $id,
			':user_id'        => session()->get('user_id'),
			':name'           => $posts['academic_name'],
	...
	public function delete(string $id): PDOStatement | false {
		$sql = "UPDATE academic_backgrounds SET delete_flg = 1 WHERE user_id = :user_id AND id = :a_id AND delete_flg = 0";
		$data = [
			':a_id'    => $id,
			':user_id' => session()->get('user_id')
		];
